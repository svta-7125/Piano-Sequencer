...
dir_path = environ['USERPROFILE']+"/Piano_Sequencer"
...
def file():
    ...
    name = input() + '.txt'
    path = environ['USERPROFILE']+"/Piano_Sequencer/"
    path += name
    full_text = file_bpm+" "+new_song
...
def run_file():
    ...
    name = input()
    print("\n")
    path = environ['USERPROFILE']+"/Piano_Sequencer/" + name + '.txt'
    with open(path) as f:
        notes = f.readlines()
    digit = ""
    for i in notes:
